...
describe("welcome automation", () => {
  ...
  async function createContact(overrides: Partial<{ email: string; firstName: string; status: "subscribed" | "unsubscribed" | "bounced" }> = {}): Promise<string> {
  ...
  function successAdapter(): { adapter: ResendEmailAdapter; sendEmail: ReturnType<typeof vi.fn> } {
    const sendEmail = vi.fn(async (_message: ResendEmailMessage) => ({ data: { id: "email_welcome_0" }, error: null }));
  ...
  describe("maybeEnqueueWelcome", () => {
    ...
    it("does not enqueue once a welcome has already been sent", async () => {
      const contactId = await createContact();
      await new CampaignRepository(env.DB).recordEvent({ contactId, type: "welcome_sent", providerEventId: "dry-run" });
  ...
  describe("processWelcomeSend", () => {
    it("sends a single welcome email with a per-contact idempotency key", async () => {
      ...
      const [message, options] = sendEmail.mock.calls[0] as [ResendEmailMessage, { idempotencyKey: string }];
      expect(message).toMatchObject({ from: "EmMail <news@example.com>", to: ["lead@example.com"] });
    ...
    it("dry-runs without calling Resend and still records welcome_sent", async () => {
      ...
      ).bind(contactId).first<{ provider_event_id: string; metadata_json: string }>();
    ...
    it("rethrows retryable Resend errors so the queue redelivers", async () => {
      ...
      const adapter: ResendEmailAdapter = {
        sendEmail: vi.fn(async () => ({ data: null, error: { name: "rate_limit_exceeded", message: "slow down" } }))
      };

      await expect(processWelcomeSend(env, { type: "welcome", contactId }, adapter)).rejects.toThrow("slow down");
      const campaigns = new CampaignRepository(env.DB);
    ...
    it("acks a terminal Resend error by recording welcome_failed (no retry storm)", async () => {
      const contactId = await createContact();
      const adapter: ResendEmailAdapter = {
        sendEmail: vi.fn(async () => ({ data: null, error: { name: "validation_error", message: "bad address" } }))
      };
      ...
      ).bind(contactId).first<{ metadata_json: string }>();
    ...
    it("treats a 409 idempotency conflict as already-sent", async () => {
      const contactId = await createContact();
      const adapter: ResendEmailAdapter = {
        sendEmail: vi.fn(async () => ({ data: null, error: { statusCode: 409, message: "conflict" } }))
      };
