...
describe("Worker API", () => {
  ...
  it("snapshots and enqueues a campaign send", async () => {
    ...
    const createResponse = await handleRequest(
      new Request("https://mail.example.com/api/campaigns", {
        method: "POST",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
          name: "June update",
    ...
    const sendResponse = await handleRequest(
      new Request(`https://mail.example.com/api/campaigns/${campaign.id}/send`, {
        method: "POST",
        headers: adminAuth,
      }),
      env
  ...
  it("returns recipient rollup stats for a campaign", async () => {
    ...
    const response = await handleRequest(
      new Request(`https://mail.example.com/api/campaigns/${campaign.id}/stats`, {
        headers: adminAuth,
      }),
      env
    );
  ...
  it("patches an existing campaign through the admin API", async () => {
    ...
    const patchResponse = await handleRequest(
      new Request(`https://mail.example.com/api/campaigns/${campaign.id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
          name: "July update",
    ...
    const missing = await handleRequest(
      new Request("https://mail.example.com/api/campaigns/cmp_missing", {
        method: "PATCH",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
          name: "Nope",
    ...
    const sending = await handleRequest(
      new Request(`https://mail.example.com/api/campaigns/${campaign.id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
  ...
  it("serves admin assets and APIs under the configured sidecar base path", async () => {
    ...
    const assetResponse = await handleRequest(
      new Request("https://southandozarks.autojack.ai/_emmail/", { headers: adminAuth }),
      env
    );
  ...
  it("protects admin assets and APIs when an admin token is configured", async () => {
    ...
    const deniedApi = await handleRequest(
      new Request("https://mail.example.com/api/contacts"),
      env
    );
    ...
    const bearerResponse = await handleRequest(
      new Request("https://mail.example.com/api/contacts", {
    ...
    const loginResponse = await handleRequest(
      new Request("https://mail.example.com/login", {
        method: "POST",
    ...
    const cookieResponse = await handleRequest(
      new Request("https://mail.example.com/api/contacts", {
        headers: { cookie },
      }),
      env
  ...
  it("seeds the welcome automation sequence through the admin API", async () => {
    const seeded = await handleRequest(
      new Request("https://mail.example.com/api/automations/seed-welcome", {
        method: "POST",
        headers: adminAuth,
      }),
      env
    ...
    const listed = await handleRequest(
      new Request("https://mail.example.com/api/automations", { headers: adminAuth }),
      env
    );
  ...
  it("creates, edits, and protects automations through the admin API", async () => {
    const createResponse = await handleRequest(
      new Request("https://mail.example.com/api/automations", {
        method: "POST",
        headers: { "content-type": "application/json", ...adminAuth },
    ...
    const saveSteps = await handleRequest(
      new Request(`https://mail.example.com/api/automations/${created.id}/steps`, {
        method: "PUT",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
          steps: [
    ...
    await expect(saveSteps.json()).resolves.toMatchObject({
      steps: expect.arrayContaining([expect.objectContaining({ stepType: "wait" })]),
    });

    const rename = await handleRequest(
      new Request(`https://mail.example.com/api/automations/${created.id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json", ...adminAuth },
    ...
    const enable = await handleRequest(
      new Request(`https://mail.example.com/api/automations/${created.id}/enable`, {
        method: "POST",
        headers: adminAuth,
      }),
      env
  ...
  it("rejects invalid automation step configs and empty enable", async () => {
    ...
    const enableEmpty = await handleRequest(
      new Request(`https://mail.example.com/api/automations/${created.id}/enable`, {
        method: "POST",
        headers: adminAuth,
      }),
  ...
  it("rejects wait steps that cannot be scheduled", async () => {
    ...
    const hugeWait = await handleRequest(
      new Request(`https://mail.example.com/api/automations/${created.id}/steps`, {
        method: "PUT",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
  ...
  it("previews an unsaved automation draft with merge tags and timing", async () => {
    const response = await handleRequest(
      new Request("https://mail.example.com/api/automations/preview", {
        method: "POST",
        headers: { "content-type": "application/json", ...adminAuth },
        body: JSON.stringify({
          firstName: "Ada",
  ...
  it("ingests South & Ozarks contact messages with list tags and idempotent events", async () => {
    ...
    const missingSecret = await handleRequest(
      new Request(
        "https://southandozarks.autojack.ai/_emmail/api/integrations/southandozarks/contact-message",
        {
          method: "POST",
  ...
  it("records signed open events and returns a transparent gif", async () => {
    ...
    const eventType = await env.DB.prepare("SELECT type FROM events WHERE recipient_id = ?")
      .bind(recipient.id)
      .first("type");
  ...
  it("handles signed unsubscribe links by creating a suppression", async () => {
    ...
    expect(response.status).toBe(200);
    const suppression = await env.DB.prepare("SELECT type FROM suppressions WHERE email = ?")
  ...
  it("seeds and clears sample data without removing real contacts", async () => {
    ...
    const seedResponse = await handleRequest(
      new Request("https://mail.example.com/api/sample-data/seed", {
        method: "POST",
        headers: adminAuth,
      }),
      env
    ...
    const contactsAfterSeed = await new ContactRepository(env.DB).listContacts({
      limit: 20,
      offset: 0,
    });
    ...
    const statusResponse = await handleRequest(
      new Request("https://mail.example.com/api/sample-data/status", { headers: adminAuth }),
      env
    );
    ...
    const automationsResponse = await handleRequest(
      new Request("https://mail.example.com/api/automations", { headers: adminAuth }),
      env
    );
    expect(automationsResponse.status).toBe(200);
    const automations = (await automationsResponse.json()) as Array<{
      slug: string;
      steps: unknown[];
    }>;
    ...
    const clearResponse = await handleRequest(
      new Request("https://mail.example.com/api/sample-data/clear", {
        method: "POST",
        headers: adminAuth,
      }),
      env
    ...
    const contactsAfterClear = await new ContactRepository(env.DB).listContacts({
      limit: 20,
      offset: 0,
    });
