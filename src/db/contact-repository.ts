...
export class ContactRepository {
  ...
  async listContacts(options: { limit: number; offset: number }): Promise<
    Array<{
      id: string;
      email: string;
      firstName: string;
  ...
  async suppressEmail(email: string, type: string, source: string, reason = ""): Promise<void> {
    ...
    await this.db
      ...
      .bind(createId("sup"), email.toLowerCase(), type, source, reason, now)
      .run();

    if (type === "unsubscribe") {
      await this.db
        .prepare("UPDATE contacts SET status = 'unsubscribed', updated_at = ? WHERE email = ?")
        .bind(now, email.toLowerCase())
        .run();
    }
    if (type === "bounce" || type === "complaint") {
      await this.db
        .prepare("UPDATE contacts SET status = 'bounced', updated_at = ? WHERE email = ?")
        .bind(now, email.toLowerCase())
        .run();
    }
  }
  ...
  async isSuppressed(email: string): Promise<boolean> {
    const row = await this.db
      .prepare("SELECT 1 AS hit FROM suppressions WHERE email = ? LIMIT 1")
      .bind(email.toLowerCase())
      .first<{ hit: number }>();
    return Boolean(row);
  }
  ...
  private async upsertContact(contact: ImportedContact): Promise<void> {
    ...
    await this.db
      ...
      .bind(contactId, contact.email, contact.firstName, contact.lastName, contact.status, now, now)
      .run();
    ...
    for (const tagName of contact.tags) {
      ...
      await this.db
        ...
        .bind(contactId, tagId, now)
        .run();
    }
  }

  private async ensureNamedRecord(
    table: "lists" | "tags",
    prefix: string,
    name: string
  ): Promise<string> {
