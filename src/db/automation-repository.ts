...
export class AutomationEmptyStepsError extends Error {
  constructor(message = "Automation must have at least one step before enabling") {
    super(message);
...
export class AutomationRepository {
  ...
  async listAutomations(): Promise<AutomationSummary[]> {
    const result = await this.db.prepare("SELECT * FROM automations ORDER BY created_at ASC").all();
  ...
  async listSteps(automationId: string): Promise<AutomationStep[]> {
    const result = await this.db
      .prepare("SELECT * FROM automation_steps WHERE automation_id = ? ORDER BY position ASC")
      .bind(automationId)
      .all();
  ...
  async getStepAt(automationId: string, position: number): Promise<AutomationStep | null> {
    const row = await this.db
      .prepare("SELECT * FROM automation_steps WHERE automation_id = ? AND position = ?")
  ...
  async updateAutomationName(id: string, name: string): Promise<AutomationSummary | null> {
    ...
    const result = await this.db
      .prepare("UPDATE automations SET name = ?, updated_at = ? WHERE id = ? AND enabled = 0")
  ...
  async replaceSteps(
    ...
    const trimmedName = options.name === undefined ? null : options.name.trim();
    ...
    for (let position = 0; position < steps.length; position += 1) {
      ...
      statements.push(
        this.db
          ...
          .bind(createId("stp"), id, position, step.stepType, JSON.stringify(step.config), now, id)
      );
    }
    statements.push(
      trimmedName
        ...
        : this.db
            .prepare("UPDATE automations SET updated_at = ? WHERE id = ? AND enabled = 0")
            .bind(now, id)
    );
  ...
  async getEnrollmentForContact(
    ...
    const row = await this.db
      .prepare("SELECT * FROM automation_enrollments WHERE automation_id = ? AND contact_id = ?")
  ...
  async ensureWelcomeSequence(): Promise<AutomationSummary> {
    ...
    for (let position = 0; position < steps.length; position += 1) {
      ...
      await this.db
        ...
        .bind(createId("stp"), automationId, position, step.type, JSON.stringify(step.config), now)
        .run();
    }
...
function assertConsecutiveNonWaitWithinGuard(steps: Array<{ stepType: string }>): void {
  let run = 0;
  for (const step of steps) {
...
function validateStepInput(step: StepInput): void {
  ...
  if (step.stepType === "send_email") {
    ...
    if (!subject || !markdownBody) {
      throw new AutomationValidationError("send_email steps require subject and markdownBody");
    }
    return;
  }
  if (step.stepType === "wait") {
  ...
  throw new AutomationValidationError(`Unknown step type: ${String(step.stepType)}`);
}
