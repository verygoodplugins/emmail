...
const sampleEvents = [
  event(
    "sample_event_send_ava",
    "sample_recipient_ava",
    "send",
    "sample_email_ava",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_send_miles",
    "sample_recipient_miles",
    "send",
    "sample_email_miles",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_send_sofia",
    "sample_recipient_sofia",
    "send",
    "sample_email_sofia",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_send_jordan",
    "sample_recipient_jordan",
    "send",
    "sample_email_jordan",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_send_casey",
    "sample_recipient_casey",
    "send",
    "sample_email_casey",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_send_taylor",
    "sample_recipient_taylor",
    "send",
    "sample_email_taylor",
    "2026-06-16T15:08:00.000Z"
  ),
  event(
    "sample_event_delivered_ava",
    "sample_recipient_ava",
    "delivered",
    "sample_email_ava",
    "2026-06-16T15:09:00.000Z"
  ),
  event(
    "sample_event_delivered_miles",
    "sample_recipient_miles",
    "delivered",
    "sample_email_miles",
  ...
  event(
    "sample_event_delivered_jordan",
    "sample_recipient_jordan",
    "delivered",
    "sample_email_jordan",
    "2026-06-16T15:11:00.000Z"
  ),
  event(
    "sample_event_opened_ava",
    "sample_recipient_ava",
    "opened",
    null,
    "2026-06-16T15:31:00.000Z"
  ),
  event(
    "sample_event_opened_sofia",
    "sample_recipient_sofia",
    "opened",
    null,
    "2026-06-16T16:04:00.000Z"
  ),
  event(
    "sample_event_opened_jordan",
    "sample_recipient_jordan",
    "opened",
    null,
    "2026-06-16T17:12:00.000Z"
  ),
  event(
    "sample_event_clicked_ava",
    "sample_recipient_ava",
    "clicked",
    null,
    "2026-06-16T15:33:00.000Z",
    "sample_link_field_notes"
  ),
  event(
    "sample_event_clicked_jordan",
    "sample_recipient_jordan",
    "clicked",
    null,
    "2026-06-16T17:14:00.000Z",
    "sample_link_events"
  ),
  event(
    "sample_event_unsubscribe_casey",
    "sample_recipient_casey",
    "unsubscribe",
    null,
  ...
  event(
    "sample_event_bounced_taylor",
    "sample_recipient_taylor",
    "bounced",
    "sample_email_taylor",
    "2026-06-16T15:12:00.000Z"
  ),
] as const;
...
export async function seedSampleData(db: D1Database): Promise<SampleDataSummary> {
  ...
  for (const list of sampleLists) {
    await db
      ...
      .bind(list.id, list.name, createdAt)
      .run();
  }
  ...
  for (const tag of sampleTags) {
    await db
      ...
      .bind(tag.id, tag.name, createdAt)
      .run();
  }
  ...
  for (const contact of sampleContacts) {
    ...
    for (const listId of contact.listIds) {
      await db
        ...
        .bind(contact.id, listId, createdAt)
        .run();
    }
    ...
    for (const tagId of contact.tagIds) {
      await db
        ...
        .bind(contact.id, tagId, createdAt)
        .run();
    }
  }
  ...
  for (const link of sampleLinks) {
    await db
      ...
      .bind(link.id, link.campaignId, link.url, link.position, createdAt)
      .run();
  }
  ...
  for (const sampleEvent of sampleEvents) {
    const link = sampleEvent.linkId
      ? sampleLinks.find((candidate) => candidate.id === sampleEvent.linkId)
      : null;
    const recipient = sampleRecipients.find(
      (candidate) => candidate.id === sampleEvent.recipientId
    );
    await db
      .prepare(
        `INSERT INTO events
...
async function countWelcomeSequence(db: D1Database): Promise<number> {
  const row = await db
    .prepare("SELECT COUNT(*) AS count FROM automations WHERE slug = ?")
    .bind("welcome-sequence")
    .first<{ count: number }>();
  return Number(row?.count ?? 0);
}
...
async function countSuppressions(db: D1Database): Promise<number> {
  ...
  const row = await db
    ...
    .bind(...suppressionIds, ...sampleEmails)
    .first<{ count: number }>();
  return Number(row?.count ?? 0);
}
...
async function deleteSampleEvents(db: D1Database): Promise<void> {
  ...
  await db
    ...
    .bind(...eventIds, ...campaignIds, ...contactIds, ...recipientIds, ...linkIds)
    .run();
}
